...
conn = sqlite3.connect("cadets-database.db") # connet to SQLite
cur = conn.cursor()

#--- objects: uuid, host(), type ---#
...
#--- subjects: uuid, host, parent, principal ---#
...
#--- Define Node type and No. ---#
node_type_list = ['FILE_OBJECT_DIR', 
                  'FILE_OBJECT_FILE', 
                  'FILE_OBJECT_UNIX_SOCKET', 
                  'IPC_OBJECT_PIPE_UNNAMED',
                  'IPC_OBJECT_SOCKET_PAIR', 
...
#--- process JSON into database---#
with open("darpa-tc-cadets-object.json", encoding="utf-8") as f:
    for line in tqdm(f):
        cadets_object = json.loads(line) # Json -> dict
